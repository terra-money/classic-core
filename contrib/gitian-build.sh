...
DEFAULT_TERRA_SIGS=${TERRA_SIGS:-'terra.sigs'}
DEFAULT_GITIAN_REPO='https://github.com/devrandom/gitian-builder'
DEFAULT_GBUILD_FLAGS=''
DEFAULT_SIGS_REPO='https://github.com/cosmos/terra.sigs'
...
The signature will be saved in '${DEFAULT_TERRA_SIGS}/'. An alternative output directory
for signatures can be supplied via the environment variable \$TERRA_SIGS.
...
f_build() {
  ...
  bin/gbuild --commit terra="$g_commit" ${GBUILD_FLAGS} "$l_descriptor"
...
g_sigs_dir=${TERRA_SIGS:-"${g_workdir}/${DEFAULT_TERRA_SIGS}"}
