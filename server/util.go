...
func interceptLoadConfig() (conf *cfg.Config, err error) {
	...
	terraConfigFilePath := filepath.Join(rootDir, "config/terrad.toml")
	if _, err := os.Stat(terraConfigFilePath); os.IsNotExist(err) {
		terraConf, _ := config.ParseConfig()
		config.WriteConfigFile(terraConfigFilePath, terraConf)
	}

	viper.SetConfigName("terrad")
