...
func TestSimAppExportAndBlockedAddrs(t *testing.T) {
	...
	for acc := range maccPerms {
		_, ok := allowedReceivingModAcc[acc]
		require.Equal(
			t,
...
func TestRunMigrations(t *testing.T) {
	...
	app := NewTerraApp(logger, db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, encCfg, simapp.EmptyAppOptions{}, wasmconfig.DefaultConfig())
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = app.mm.RunMigrations(
				...
				module.VersionMap{
					...
					"market":                 market.AppModule{}.ConsensusVersion(),
