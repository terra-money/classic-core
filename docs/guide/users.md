...
The address is a public string with a human-readable prefix \(e.g. `terra10snjt8dmpr5my0h76xj48ty80uzwhraqmmx4mg`\) that identifies your account. When someone wants to send you funds, they send it to your address. It is computationally infeasible to find the private key associated with a given address.
...
{% hint style="warning" %}
On Terra mainnet, the accepted denom is `uluna`, where `1luna = 1,000,000uluna`
{% endhint %}

### A note on gas, fees and taxes

Transactions on the Terra Protocol network need to include a transaction fee in order to be processed. This fee pays for the gas and taxes required to run the transaction. The formula is the following:

```text
fees = ceil(gas * gasPrices + min(stableAmt * taxRate, taxCap))
```

Where `stableAmt` is the quantity of stable coins excpet luna.
...
The `tax` is a fee used for the stability mechanism and is only charged for `bank/MsgSend` transactions that contain a stable coin. The `tax` is capped at some fixed amount of SDT to avoid penalizing large transactions.

{% hint style="info" %}
For mainnet, the recommended `gas-prices` is `0.015uluna`. 
{% endhint %}
