...
// LazyVestingSchedule maps the ratio of tokens that becomes vested by blocktime (in seconds) from genesis.
...
// NewLazyVestingSchedule creates a new vesting lazy schedule instance.
func NewLazyVestingSchedule(denom string, lazySchedules []LazySchedule) LazyVestingSchedule {
	return LazyVestingSchedule{
		Denom:         denom,
...
func (vs LazyVestingSchedule) GetVestedRatio(blockTime int64) sdk.Dec {
...
func (vs LazyVestingSchedule) GetDenom() string {
	return vs.Denom
}

// IsValid checks that the vesting lazy schedule is valid.
func (vs LazyVestingSchedule) IsValid() bool {
	sumRatio := sdk.ZeroDec()
	for _, lazySchedule := range vs.LazySchedules {
...
// String implements the fmt.Stringer interface
func (vs LazyVestingSchedule) String() string {
	return fmt.Sprintf(`LazyVestingSchedule:
	Denom: %v,
	LazySchedules: %v`,
...
type LazyGradedVestingAccount interface {
	...
	GetLazyVestingSchedules() []LazyVestingSchedule
	GetLazyVestingSchedule(denom string) (LazyVestingSchedule, bool)
}
...
func NewBaseLazyGradedVestingAccount(baseAcc *auth.BaseAccount, lazyVestingSchedules []LazyVestingSchedule) *BaseLazyGradedVestingAccount {
...
// GetLazyVestingSchedules returns the LazyVestingSchedules of the graded lazy vesting account
func (lgva BaseLazyGradedVestingAccount) GetLazyVestingSchedules() []LazyVestingSchedule {
	return lgva.LazyVestingSchedules
}

// GetLazyVestingSchedule returns the VestingSchedule of the given denom
func (lgva BaseLazyGradedVestingAccount) GetLazyVestingSchedule(denom string) (LazyVestingSchedule, bool) {
	for _, vs := range lgva.LazyVestingSchedules {
		if vs.Denom == denom {
			return vs, true
		}
	}

	return LazyVestingSchedule{}, false
}
...
func (lgva BaseLazyGradedVestingAccount) GetVestedCoins(blockTime time.Time) sdk.Coins {
	...
	for _, ovc := range lgva.OriginalVesting {
		if vestingSchedule, exists := lgva.GetLazyVestingSchedule(ovc.Denom); exists {
