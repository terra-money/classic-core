	...
	for i, acc := range simState.Accounts {
		...
		if int64(i) > simState.NumBonded && simState.Rand.Intn(100) < 50 {
			...
			for i := int64(0); i < scheduleNum; i++ {
				...
				lazySchedules = append(lazySchedules, vestingtypes.NewLazySchedule(startTime, endTime, ratio))
			}

			gacc = vestingtypes.NewLazyGradedVestingAccount(&bacc, vestingtypes.VestingSchedules{
				vestingtypes.NewVestingSchedule(sdk.DefaultBondDenom, lazySchedules),
			})
		}
