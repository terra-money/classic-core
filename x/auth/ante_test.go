...
func TestAnteHandlerSigErrors(t *testing.T) {
	...
	ctx := input.ctx
	anteHandler := NewAnteHandler(input.ak, input.fck, NewMockTreasuryKeeper())
...
func TestAnteHandlerFees(t *testing.T) {
	...
	privs, accnums, seqs := []crypto.PrivKey{priv1}, []uint64{0}, []uint64{0}
	fee := newStdFee()
	msgs := []sdk.Msg{msg, bank.NewMsgSend(addr1, addr2, sdk.NewCoins(sdk.NewInt64Coin(assets.MicroSDRDenom, 1000000)))}

	// signer does not have enough funds to pay the fee
	tx = newTestTx(ctx, msgs, privs, accnums, seqs, fee)
	checkInvalidTx(t, anteHandler, ctx, tx, false, sdk.CodeInsufficientFee)

	fee.Amount = sdk.NewCoins(sdk.NewInt64Coin(assets.MicroSDRDenom, 1000))
	tx = newTestTx(ctx, msgs, privs, accnums, seqs, fee)
	acc1.SetCoins(sdk.NewCoins(sdk.NewInt64Coin(assets.MicroSDRDenom, 999)))
	input.ak.SetAccount(ctx, acc1)
	...
	require.True(t, input.ak.GetAccount(ctx, addr1).GetCoins().AmountOf(assets.MicroSDRDenom).Equal(sdk.NewInt(999)))

	acc1.SetCoins(sdk.NewCoins(sdk.NewInt64Coin(assets.MicroSDRDenom, 1000)))
	input.ak.SetAccount(ctx, acc1)
	checkValidTx(t, anteHandler, ctx, tx, false)

	require.True(t, input.fck.GetCollectedFees(ctx).IsEqual(sdk.NewCoins(sdk.NewInt64Coin(assets.MicroSDRDenom, 1000))))
	require.True(t, input.ak.GetAccount(ctx, addr1).GetCoins().AmountOf(assets.MicroSDRDenom).Equal(sdk.NewInt(0)))
}
...
func TestAnteHandlerMemoGas(t *testing.T) {
	...
	msg := newTestMsg(addr1)
	privs, accnums, seqs := []crypto.PrivKey{priv1}, []uint64{0}, []uint64{0}
	fee := auth.NewStdFee(0, sdk.NewCoins(sdk.NewInt64Coin(assets.MicroSDRDenom, 0)))
	...
	fee = auth.NewStdFee(801, sdk.NewCoins(sdk.NewInt64Coin(assets.MicroSDRDenom, 0)))
	tx = newTestTxWithMemo(ctx, []sdk.Msg{msg}, privs, accnums, seqs, fee, "abcininasidniandsinasindiansdiansdinaisndiasndiadninsd")
	checkInvalidTx(t, anteHandler, ctx, tx, false, sdk.CodeOutOfGas)

	// memo too large
	fee = auth.NewStdFee(9000, sdk.NewCoins(sdk.NewInt64Coin(assets.MicroSDRDenom, 0)))
...
func TestProcessPubKey(t *testing.T) {
	...
	tests := []struct {
		...
		{"no sigs, account with pub, simulate on", args{acc2, auth.StdSignature{}, true}, false},
...
func TestConsumeSignatureVerificationGas(t *testing.T) {
	...
	tests := []struct {
		...
		{"PubKeyEd25519", args{sdk.NewInfiniteGasMeter(), nil, ed25519.GenPrivKey().PubKey(), params}, auth.DefaultSigVerifyCostED25519, true},
		{"PubKeySecp256k1", args{sdk.NewInfiniteGasMeter(), nil, secp256k1.GenPrivKey().PubKey(), params}, auth.DefaultSigVerifyCostSecp256k1, false},
...
func expectedGasCostByKeys(pubkeys []crypto.PubKey) uint64 {
	...
	for _, pubkey := range pubkeys {
		...
		case strings.Contains(pubkeyType, "ed25519"):
			cost += auth.DefaultParams().SigVerifyCostED25519
		case strings.Contains(pubkeyType, "secp256k1"):
			cost += auth.DefaultParams().SigVerifyCostSecp256k1
		default:
...
func TestEnsureSufficientMempoolFees(t *testing.T) {
	...
	testCases := []struct {
		...
		{auth.NewStdFee(200000, sdk.NewCoins(sdk.NewInt64Coin("photino", 5))), false},
		{auth.NewStdFee(200000, sdk.NewCoins(sdk.NewInt64Coin("stake", 1))), false},
		{auth.NewStdFee(200000, sdk.NewCoins(sdk.NewInt64Coin("stake", 2))), true},
		{auth.NewStdFee(200000, sdk.NewCoins(sdk.NewInt64Coin("photino", 10))), true},
		{
			auth.NewStdFee(
				200000,
				sdk.NewCoins(
		...
		},
		{
			auth.NewStdFee(
				200000,
				sdk.NewCoins(
					sdk.NewInt64Coin(assets.MicroSDRDenom, 5),
					sdk.NewInt64Coin("photino", 10),
					sdk.NewInt64Coin("stake", 2),
				),
			),
	...
	for i, tc := range testCases {
		res := EnsureSufficientMempoolFees(ctx, tc.input, sdk.Coins{})
		require.Equal(
