...
func registerQueryRoutes(cliCtx context.CLIContext, r *mux.Router) {
	...
	r.HandleFunc("/market/luna_pool", queryLunaPoolHandlerFn(cliCtx)).Methods("GET")
...
func queryLunaPoolHandlerFn(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		cliCtx, ok := rest.ParseQueryHeightOrReturnBadRequest(w, cliCtx, r)
		if !ok {
			return
		...
		res, height, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryLunaPool), nil)
		if err != nil {
