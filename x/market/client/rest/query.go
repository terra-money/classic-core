...
func registerQueryRoutes(cliCtx context.CLIContext, r *mux.Router) {
	...
	r.HandleFunc("/market/last_day_issuance", queryLastDayIssuanceHandlerFn(cliCtx)).Methods("GET")
...
func queryLastDayIssuanceHandlerFn(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		cliCtx, ok := rest.ParseQueryHeightOrReturnBadRequest(w, cliCtx, r)
		if !ok {
			return
		...
		res, height, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryLastDayIssuance), nil)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusNotFound, err.Error())
			return
		}
