...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"send tokens": {
			...
			resultAssertions: []assertion{assertReturnedEvents(5), assertGasUsed(138000, 139000)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
			...
			resultAssertions: []assertion{assertGasUsed(99000, 100000), assertErrorString("insufficient funds")},
		},
