...
func TestLegacyDispatchSubMsgSuccessCase(t *testing.T) {
...
func TestLegacyDispatchSubMsgErrorHandling(t *testing.T) {
	...
	hackatomCode, err := ioutil.ReadFile("./testdata/hackatom_legacy.wasm")
	require.NoError(t, err)
	...
	cases := map[string]struct {
		...
		"send tokens": {
			...
			resultAssertions: []assertion{assertReturnedEvents(5), assertGasUsed(172000, 173000)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
			...
			resultAssertions: []assertion{assertGasUsed(138000, 139000), assertErrorString("insufficient funds")},
		},
		...
		"send tokens with limit": {
			...
			resultAssertions: []assertion{assertReturnedEvents(5), assertGasUsed(172800, 173000)},
		},
		...
		"out of gas caught with gas limit": {
			...
			resultAssertions: []assertion{assertGasUsed(subGasLimit+131000, subGasLimit+133000), assertErrorString("out of gas")},
		},
...
// Try a simple send, no gas limit to for a sanity check before trying table tests
func TestLegacyDispatchSubMsgConditionalReplyOn(t *testing.T) {
	input := CreateTestInput(t)
	ctx, accKeeper, keeper, bankKeeper := input.Ctx, input.AccKeeper, input.WasmKeeper, input.BankKeeper
