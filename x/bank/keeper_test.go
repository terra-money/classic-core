...
func TestViewKeeper(t *testing.T) {
	...
	// Test GetIssuance
	require.Equal(t, viewKeeper.GetIssuance(ctx, "foocoin").Int64(), int64(10))

	bankKeeper.AddCoins(ctx, addr, sdk.Coins{sdk.NewInt64Coin("foocoin", 100)})
	bankKeeper.SubtractCoins(ctx, addr, sdk.Coins{sdk.NewInt64Coin("foocoin", 30)})
	bankKeeper.AddCoins(ctx, addr, sdk.Coins{sdk.NewInt64Coin("barcoin", 100)})

	require.Equal(t, viewKeeper.GetIssuance(ctx, "foocoin").Int64(), int64(80))
	require.Equal(t, viewKeeper.GetIssuance(ctx, "barcoin").Int64(), int64(100))
}
