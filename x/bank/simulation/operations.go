...
func SimulateMsgSend(ak types.AccountKeeper, bk bank.Keeper) simulation.Operation {
	return func(
		...
		if !bk.GetSendEnabled(ctx) {
			return simulation.NoOpMsg(bank.ModuleName), nil, nil
		}

		simAccount, toSimAcc, coins, skip, err := randomSendFields(r, ctx, accs, ak)
		if err != nil {
			return simulation.NoOpMsg(bank.ModuleName), nil, err
		}
		...
		msg := bank.NewMsgSend(simAccount.Address, toSimAcc.Address, coins)
...
func sendMsgSend(
	...
	msg bank.MsgSend, ctx sdk.Context, chainID string, privkeys []crypto.PrivKey,
) error {
...
func SimulateMsgMultiSend(ak types.AccountKeeper, bk bank.Keeper) simulation.Operation {
	return func(
		...
		inputs := make([]bank.Input, r.Intn(3)+1)
		outputs := make([]bank.Output, r.Intn(3)+1)
		...
		for i := range inputs {
			...
			inputs[i] = bank.NewInput(simAccount.Address, coins)
		...
		for o := range outputs {
			...
			outputs[o] = bank.NewOutput(outAddr.Address, outCoins)
		}
...
func sendMsgMultiSend(
	r *rand.Rand, app *baseapp.BaseApp, ak types.AccountKeeper,
	msg bank.MsgMultiSend, ctx sdk.Context, chainID string, privkeys []crypto.PrivKey,
) error {
