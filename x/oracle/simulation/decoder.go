...
func NewDecodeStore(cdc codec.Marshaler) func(kvA, kvB kv.Pair) string {
	return func(kvA, kvB kv.Pair) string {
		...
		case bytes.Equal(kvA.Key[:1], types.MissCounterKey):
			...
			cdc.MustUnmarshalBinaryBare(kvA.Value, &counterA)
			cdc.MustUnmarshalBinaryBare(kvB.Value, &counterB)
		...
		case bytes.Equal(kvA.Key[:1], types.TobinTaxKey):
			...
			cdc.MustUnmarshalBinaryBare(kvA.Value, &tobinTaxA)
			cdc.MustUnmarshalBinaryBare(kvB.Value, &tobinTaxB)
