...
func TestPBPower(t *testing.T) {
	...
	for i := 0; i < len(sk.Validators()); i++ {
		vote := NewVote(sdk.ZeroDec(), core.MicroSDRDenom, sdk.ValAddress(valAccAddrs[i]))
		pb = append(pb, vote)
	...
	fakeVote := NewVote(sdk.OneDec(), core.MicroSDRDenom, faceValAddr)
...
func TestPBWeightedMedian(t *testing.T) {
	...
	for _, tc := range tests {
		...
		for i, input := range tc.inputs {
			...
			vote := NewVote(sdk.NewDecWithPrec(int64(input*base), int64(oracleDecPrecision)), core.MicroSDRDenom, valAddr)
			pb = append(pb, vote)
		}
...
func TestString(t *testing.T) {
	...
	vote := NewVote(sdk.NewDecWithPrec(int64(1123400), int64(oracleDecPrecision)), core.MicroSDRDenom, sdk.ValAddress{})
	pb = append(pb, vote)
...
// 		pb := ExchangeRateBallot{}
// 		for i, input := range tc.inputs {
// 			vote := NewVote(sdk.NewDecWithPrec(int64(input*100), 2), "",
