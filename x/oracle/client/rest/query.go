...
func registerQueryRoute(cliCtx context.CLIContext, r *mux.Router) {
	...
	r.HandleFunc(fmt.Sprintf("/oracle/denoms/{%s}/exchange-rate", RestDenom), queryExchangeRateHandlerFunction(cliCtx)).Methods("GET")
	r.HandleFunc("/oracle/denoms/actives", queryActivesHandlerFunction(cliCtx)).Methods("GET")
	r.HandleFunc("/oracle/denoms/exchange-rates", queryExchangeRatesHandlerFunction(cliCtx)).Methods("GET")
